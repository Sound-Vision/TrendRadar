...
# AGENT_COMMAND="${AGENT_COMMAND:-claude --dangerously-skip-permissions -p /collect_tech_topic}"
# AGENT_COMMAND="${AGENT_COMMAND:-claude --dangerously-skip-permissions --model claude-sonnet-4-6 --effort low -p /collect_tech_topic}"
# AGENT_COMMAND="${AGENT_COMMAND:-gemini -m gemini-3.1-pro-preview -p /collect_tech_topic -y}"
# AGENT_COMMAND="${AGENT_COMMAND:-gemini -p /collect_tech_topic -y}"
AGENT_COMMAND="${AGENT_COMMAND:-codex exec \$collect_tech_topics --yolo}"
